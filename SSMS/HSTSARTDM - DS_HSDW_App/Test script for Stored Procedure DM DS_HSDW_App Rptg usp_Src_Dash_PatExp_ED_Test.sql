...
SELECT er.Event_Type,
       ...
       er.Event_Date,
       er.Event_Date_Disch,
       er.sk_Dim_PG_Question,
       er.VARNAME,
       er.QUESTION_TEXT_ALIAS,
       er.Recvd_Date,
       er.Discharge_Date,
       er.Phys_Name,
       er.Phys_Dept,
...
FROM #ED er
INNER JOIN DS_HSDW_Prod.Rptg.vwDim_Date ddte
ON er.Event_Date = ddte.day_date
WHERE er.Event_FY = 2019
AND er.SURVEY_ID IS NOT NULL
AND er.AGE_STATUS IS NOT NULL
ORDER BY Event_Type
       , Event_FY
	   , UNIT
	   , AGE_STATUS
